# Spec: 秘密管理 (Secrets Management)

## 概述

實作完整的秘密管理系統,防止 API 金鑰和敏感資訊洩漏。
...
```
❌ 環境變數驗證失敗:
  - OPENAI_API_KEY: 缺少必要的環境變數
  - SUPABASE_SERVICE_ROLE_KEY: 格式無效 (應為 JWT 格式)

請檢查您的 .env.local 檔案,並參考 .env.example
```

## 相關 Specs

- `logging-security`: 日誌不應包含秘密
- `automated-security-checks`: 自動偵測秘密洩漏
