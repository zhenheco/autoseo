...
export class MultiAgentOutputAdapter {
  adapt(input: AdapterInput): WritingAgentOutput {
    ...
    return {
      ...
      statistics: {
        ...
        readingTime: this.calculateReadingTime(assemblerOutput.statistics.totalWords),
  ...
  private calculateReadability(markdown: string): ReadabilityMetrics {
    ...
    const complexWords = words.filter(word => this.syllableCount(word) > 2).length;
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const job of jobs) {
      ...
      // 檢查超時（> 30 分鐘）
      if (totalDuration > 30) {
        console.log(`[Monitor] ⏰ Job ${job.id} timeout (${totalDuration.toFixed(1)}min)`);
