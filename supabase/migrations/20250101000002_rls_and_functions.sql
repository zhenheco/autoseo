...
-- 創建加密密鑰
INSERT INTO pgsodium.key (name, status, key_type, key_id, key_context, created, expires, associated_data)
SELECT
  'wordpress_tokens',
  'valid',
  'aead-det'::pgsodium.key_type,
  gen_random_uuid(),
  'wordpress_tokens'::bytea,
  NOW(),
  NULL,
  NULL
WHERE NOT EXISTS (
  SELECT 1 FROM pgsodium.key WHERE name = 'wordpress_tokens'
);
...
SELECT
  ...
  NULL
WHERE NOT EXISTS (
  SELECT 1 FROM pgsodium.key WHERE name = 'api_keys'
);

-- 加密函數
CREATE OR REPLACE FUNCTION encrypt_data(plaintext TEXT, key_name TEXT)
RETURNS TEXT AS $$
DECLARE
  key_id UUID;
BEGIN
...
-- 解密函數
CREATE OR REPLACE FUNCTION decrypt_data(ciphertext TEXT, key_name TEXT)
RETURNS TEXT AS $$
DECLARE
  key_id UUID;
BEGIN
