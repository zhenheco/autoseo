-- 修正 Token 扣款順序：先扣單次購買的 credits (purchased_token_balance)，再扣月配額 (monthly_quota_balance)
-- 用戶需求：「扣款的時候要先用單次的扣款」
