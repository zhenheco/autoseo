...
-- Agent 執行記錄的存取控制
CREATE POLICY "agent_executions_select_own_company"
...
CREATE POLICY "agent_executions_insert_system"
...
-- 註解
COMMENT ON TABLE ai_models IS 'AI 模型配置表，儲存所有可用的 AI 模型資訊';
COMMENT ON TABLE agent_executions IS 'Agent 執行記錄表，用於監控和分析 Agent 效能';
COMMENT ON COLUMN companies.ai_model_preferences IS '公司的 AI 模型偏好設定';
COMMENT ON FUNCTION get_company_ai_models IS '取得公司設定的 AI 模型資訊';
COMMENT ON FUNCTION get_agent_execution_stats IS '取得 Agent 執行統計資料';
