...
export async function POST(request: Request) {
  ...
  const balance = await billingService.getCompanyBalance(session.user.companyId);
  ...
  // 2. 建立文章任務
  const { data: job } = await supabase.from('article_jobs').insert({
...
async function main() {
  ...
  try {
    ...
    if (result.idempotent) {
      console.log('Note: This was a duplicate request, no additional tokens deducted');
    }
  } catch (error) {
  ...
  for (const record of stuckRecords) {
    try {
      ...
      if (article) {
        ...
        // 文章不存在，標記為 failed
        console.log(`Article not found for record ${record.id}, marking as failed`);
