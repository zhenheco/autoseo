...
```typescript
function resolveTimeConflicts(schedules: Map<string, Date>): Map<string, Date> {
...
async function generateInternalLinks(
  ...
  // 2. 使用 AI 分析相關性
  const relatedArticles = await findRelatedArticles(content, publishedArticles);
  ...
  const results = await processBatchWithLimit(
    ...
    async (plan: PublishPlan) => {
      try {
        ...
        // 3.2 判斷文章類型（若未填）
        const articleType =
          plan.articleType || (await determineArticleType(plan.keyword));
