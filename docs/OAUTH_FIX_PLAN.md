...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    ...
    if (isOAuth) {
      console.log('[OAuth Callback] OAuth login detected, ensuring company setup')
...
**使用**：
```typescript
import { createErrorRedirect, isSafeRedirect } from '@/lib/auth/redirect-helpers'
...
**代碼**：
```typescript
const USE_NEW_OAUTH_FLOW = process.env.NEXT_PUBLIC_USE_NEW_OAUTH_FLOW === 'true'
...
export const logger = {
  oauth: {
    callbackStart: (code: string) =>
      log('info', 'OAuth callback started', { code: code.slice(0, 10) + '...' }),
    ...
    error: (step: string, error: any) =>
      log('error', `Error at ${step}`, { error: error.message, stack: error.stack })
  }
}
...
class OAuthMetrics {
  ...
  recordSuccess(duration: number, usedFallback: boolean) {
    ...
    // 計算移動平均
    const total = this.metrics.averageTime * (this.metrics.success - 1) + duration
  ...
  getReport() {
    return {
      ...
      fallbackRate: (this.metrics.fallbackUsed / this.metrics.success * 100).toFixed(2) + '%'
    }
  }
}
