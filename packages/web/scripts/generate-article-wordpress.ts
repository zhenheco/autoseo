...
async function generateArticle() {
  ...
  if (websitesError || !websites || websites.length === 0) {
    ...
    console.error(
      "提示: 請確保 website_configs 表中有啟用的網站（is_active=true, wp_enabled=true）",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      if (result.category) {
        ...
        console.log(
          "分類:",
          result.category.categories.map((c) => c.name).join(", "),
        );
        console.log(
          "標籤:",
          result.category.tags.map((t) => t.name).join(", "),
        );
      }
    ...
    if (result.executionStats) {
      ...
      console.log(
        "總時間:",
        (result.executionStats.totalTime / 1000).toFixed(2),
        "秒",
      );
