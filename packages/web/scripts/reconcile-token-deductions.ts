...
async function reconcileStuckDeductions() {
  ...
  console.log(
    `[Reconciliation] Found ${stuckRecords?.length || 0} stuck deduction records`,
  );
  ...
  for (const record of stuckRecords || []) {
    try {
      if (record.article_id) {
        ...
        if (article) {
          console.log(
            `[Reconciliation] Retrying deduction for record ${record.id}`,
          );
      ...
      console.error(
        `[Reconciliation] Failed to reconcile record ${record.id}:`,
        error,
      );
    }
  }
...
async function retryStuckDeduction(
  record: Database["public"]["Tables"]["token_deduction_records"]["Row"],
) {
  const { error } = await supabase.rpc("deduct_tokens_atomic", {
  ...
  if (error) {
    console.error(
      `[Reconciliation] Retry failed for record ${record.id}:`,
      error.message,
    );
    await supabase
      .from("token_deduction_records")
