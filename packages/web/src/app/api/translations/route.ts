...
export const POST = withRouteAuth(
  ...
  async (request: NextRequest, { user }) => {
    ...
    // 解析請求
    const jsonResult = await safeJson<CreateTranslationJobRequest>(request);
    ...
    try {
      ...
      if (!companyScope.success) {
        if (companyScope.reason === "query_failed") {
          return internalError("Failed to fetch company membership");
        }
