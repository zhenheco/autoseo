...
export class SubscriptionService {
  ...
  async createTokenSubscription(
    ...
    if (subscriptionError) {
      console.error(
        "[SubscriptionService] 建立月訂閱 Token 失敗:",
        subscriptionError,
      );
    ...
    if (subscription) {
      const newBalance =
        subscription.purchased_token_balance + planData.base_tokens;
  ...
  async upgradePlan(
    ...
    const tokenDiff =
      newPlan.base_tokens - currentSubscription.monthlyTokenQuota;
  ...
  async purchaseTokenPackage(
    ...
    return { success: false, error: "付款功能暫時停用" };
  }
}
