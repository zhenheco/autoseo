...
export function TokenBalanceDisplay({ compact = false }: TokenBalanceDisplayProps) {
  ...
  if (isLoading) {
    return (
      <div className={`flex items-center gap-2 ${compact ? '' : 'rounded-lg border border-border bg-background px-3 py-2'}`}>
  ...
  if (error || !balance) {
    return (
      <div className={`flex items-center gap-2 ${compact ? '' : 'rounded-lg border border-destructive/50 bg-destructive/10 px-3 py-2'}`}>
        <p className="text-xs text-destructive">{error?.message || '無法載入餘額'}</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-border bg-background px-3 py-2 shadow-sm transition-all duration-300 hover:shadow-md">
      <div className="flex items-center gap-2">
        <span className="text-sm font-medium text-muted-foreground">Credit 餘額:</span>
        <span
      ...
      <div className="flex items-center gap-3 border-l border-border pl-3 text-xs text-muted-foreground">
        <span className="transition-all duration-300 ease-in-out">
          月配額: <span className="font-medium transition-colors duration-300">{formatNumber(balance.monthly)}</span>
        </span>
        {balance.purchased > 0 && (
          <span className="transition-all duration-300 ease-in-out">
            購買: <span className="font-medium transition-colors duration-300">{formatNumber(balance.purchased)}</span>
          </span>
        )}
      </div>
