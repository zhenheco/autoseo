...
import { BANK_CODES, MIN_WITHDRAWAL_AMOUNT } from '@/types/affiliate.types'
    ...
    <div className="container mx-auto max-w-2xl space-y-6 p-6">
      ...
      <Card className="border-green-200 bg-green-50">
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            <span className="text-lg font-medium text-green-900">可提領佣金</span>
            <span className="text-3xl font-bold text-green-600">
              NT$ {pendingCommission.toLocaleString()}
            </span>
          </div>
        </CardContent>
      </Card>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>最低提領金額 NT${MIN_WITHDRAWAL_AMOUNT}</CardDescription>
          </CardHeader>
            ...
            {amount >= MIN_WITHDRAWAL_AMOUNT && (
              <div className="rounded-md bg-blue-50 p-4 space-y-2">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-blue-800">扣繳稅額 ({taxRate}%)：</span>
                    <span className="text-red-600">-NT$ {calculateTax(amount).toLocaleString()}</span>
                  </div>
                  <div className="border-t border-blue-200 pt-1 mt-1"></div>
                  <div className="flex justify-between">
                    <span className="font-semibold text-blue-900">實際入帳：</span>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>請填寫正確的銀行帳戶，戶名需與身份證相符</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">
        ...
        <Card className="border-yellow-200 bg-yellow-50">
          <CardContent className="pt-6">
            ...
            <ul className="list-disc list-inside space-y-1 text-sm text-yellow-800">
              <li>提領申請提交後，預計 5-7 個工作天內完成審核並撥款</li>
              <li>首次提領需上傳身份證和銀行存摺封面以完成驗證</li>
        ...
        <div className="flex gap-4">
          <Button type="submit" disabled={loading || amount < MIN_WITHDRAWAL_AMOUNT} className="flex-1">
            {loading ? '處理中...' : '提交申請'}
          </Button>
          <Button
            type="button"
