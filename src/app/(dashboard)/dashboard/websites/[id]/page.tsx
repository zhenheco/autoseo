...
async function getWebsiteArticles(websiteId: string, companyId: string) {
  ...
  const articles = (data || []).map(article => ({
    ...article,
    article_job: Array.isArray(article.article_job) && article.article_job.length > 0
...
export default async function WebsiteDetailPage({
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      {/* 已發布文章 */}
      <div className="mb-4">
        <h2 className="text-2xl font-bold">已發布文章</h2>
        <p className="text-sm text-muted-foreground mt-1">已發布到此網站的文章存檔</p>
      </div>
      ...
      {articles.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground">此網站尚無已發布的文章</p>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {article.article_job?.scheduled_publish_at && (
                      <Badge variant="outline" className="text-xs whitespace-nowrap">
                        ...
                        {formatScheduleTime(article.article_job.scheduled_publish_at)}
                      </Badge>
                    )}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>{article.reading_time || 0} 分鐘</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Calendar className="h-3 w-3" />
                      <span className="hidden sm:inline">{formatDate(article.created_at)}</span>
                      <span className="sm:hidden">{formatScheduleTime(article.created_at)}</span>
                    </div>
                  </div>
