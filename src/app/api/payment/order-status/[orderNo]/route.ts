...
export const GET = withRouteAuth(
  ...
  async (
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 5; attempt++) {
        ...
        if (orderNo.startsWith("MAN")) {
          ...
          if (mandate && !error && mandate.payment_orders) {
            orderData =
              mandate.payment_orders as Database["public"]["Tables"]["payment_orders"]["Row"];
            lastError = null;
            console.log(
              `[Order Status API] 找到定期定額委託 (嘗試 ${attempt}/5):`,
              {
                mandateNo: orderNo,
          ...
          lastError = error;
          console.log(`[Order Status API] 查詢委託失敗 (嘗試 ${attempt}/5):`, {
            orderNo,
            error,
          });
        } else {
          ...
          lastError = error;
          console.log(`[Order Status API] 查詢訂單失敗 (嘗試 ${attempt}/5):`, {
            orderNo,
            error,
          });
        }
      ...
      if (!orderData) {
        console.log("[Order Status API] 訂單尚未同步:", {
          orderNo,
          error: lastError,
        });
