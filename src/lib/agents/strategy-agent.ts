...
Please output the complete JSON that conforms to the above schema in ${languageName}:`;
      ...
      // 驗證並解析 JSON
      const parsed = this.validateAndParseOutline(response.content, outlineSchema);

      if (parsed) {
        console.log('[StrategyAgent] ✅ Successfully generated and validated outline from AI');
        return parsed;
      }
      ...
      console.warn('[StrategyAgent] AI outline generation failed, using fallback:', error);
    ...
    return {
      ...
      properties: {
        ...
        mainSections: {
          ...
          items: {
            type: 'object',
            required: ['heading', 'subheadings', 'keyPoints', 'targetWordCount', 'keywords'],
            properties: {
              heading: { type: 'string', description: '章節標題' },
              subheadings: { type: 'array', items: { type: 'string' }, description: '子標題（2-3個）' },
              keyPoints: { type: 'array', items: { type: 'string' }, description: '重點（2-4個）' },
              targetWordCount: { type: 'number', description: '目標字數' },
              keywords: { type: 'array', items: { type: 'string' }, description: '關鍵字（2-5個）' }
            }
          }
        },
        conclusion: {
   ...
   */
  private validateAndParseOutline(content: string, schema: any): StrategyOutput['outline'] | null {
    try {
      // 清理可能的 markdown 包裹
      ...
      // 基本結構驗證
      if (!parsed.introduction || !parsed.mainSections || !parsed.conclusion || !parsed.faq) {
        console.warn('[StrategyAgent] Missing required fields in outline');
        return null;
      }

      if (!Array.isArray(parsed.mainSections) || parsed.mainSections.length < 2 || parsed.mainSections.length > 4) {
        console.warn('[StrategyAgent] Invalid mainSections array length:', parsed.mainSections?.length);
        return null;
      }

      // 驗證每個 section 的必要欄位
      for (const section of parsed.mainSections) {
        if (!section.heading || !section.subheadings || !section.keyPoints || !section.targetWordCount || !section.keywords) {
          console.warn('[StrategyAgent] Section missing required fields:', section);
          return null;
        }
      }
