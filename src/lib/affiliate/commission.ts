  ...
  try {
    ...
    if (affiliate.status !== "active") {
      ...
      if (
        ...
        console.log(
          `聯盟夥伴 ${affiliateCode} 超過 3 個月無活動，但有新付款，重新計算佣金`,
        );
      }
    }
    ...
    if (!referral) {
      ...
      await supabase
        ...
        .update({
          ...
          lifetime_value:
            parseFloat(referral.lifetime_value.toString()) +
            paymentOrder.amount,
    ...
    await supabase
      ...
      .update({
        locked_commission:
          parseFloat((affiliate.locked_commission ?? 0).toString()) +
          netCommission,
        lifetime_commission:
    ...
    await supabase
      ...
      .update({
        total_commission_generated:
          parseFloat(referral.total_commission_generated?.toString() || "0") +
          commissionAmount,
      })
      .eq("id", referral.id);
...
export async function checkInactiveAffiliates(
  ...
  const { data: affiliates, error } = await supabase
    ...
    .eq("status", "active")
    .or(
      `last_active_payment_at.is.null,last_active_payment_at.lt.${threeMonthsAgo.toISOString()}`,
    );
