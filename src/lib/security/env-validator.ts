...
const ENV_VARS: EnvVar[] = [
  ...
  },
  {
    ...
    required: false,
    description: '藍新金流商店代號',
  },
  {
    name: 'NEWEBPAY_HASH_KEY',
    required: false,
    description: '藍新金流 HashKey',
  },
  {
    name: 'NEWEBPAY_HASH_IV',
    required: false,
    description: '藍新金流 HashIV',
  },
  {
...
export function validateEnv(throwOnError: boolean = true): ValidationResult {
  ...
  for (const envVar of ENV_VARS) {
    ...
    if (!value) {
      if (envVar.required) {
        errors.push(`❌ 缺少必要環境變數: ${envVar.name} - ${envVar.description}`)
      } else {
        warnings.push(`⚠️  缺少可選環境變數: ${envVar.name} - ${envVar.description}`)
      }
      continue
    }
