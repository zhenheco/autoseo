...
export const ESTIMATED_TOKENS_PER_ARTICLE = 3000;
...
export class TokenBillingService {
  ...
  async checkTokenBalance(
    ...
    const result = await this.calculator.hasEnoughTokens(
      companyId,
      estimatedTokens,
    );
  ...
  async completeWithBilling(
    ...
    usageType:
      ...
      | "title_generation"
      | "image_description"
      | "perplexity_analysis" = "article_generation",
    ...
    const balanceCheck = await this.checkTokenBalance(
      companyId,
      estimateResult.chargedTokens,
    );

    if (!balanceCheck.sufficient) {
      throw new Error(
        `Credit 餘額不足。當前: ${balanceCheck.currentBalance}, 需要: ${balanceCheck.requiredTokens}, 差額: ${balanceCheck.deficit}`,
      );
    }
    ...
    const { error: usageError } = await this.supabase
      ...
      .insert({
        ...
        metadata:
          metadata as unknown as Database["public"]["Tables"]["token_usage_logs"]["Insert"]["metadata"],
      });
  ...
  private async updateMonthlyStats(
    ...
    const { data: existingStats } = await this.supabase
      ...
      .select<
        "*",
        Database["public"]["Tables"]["monthly_token_usage_stats"]["Row"]
      >("*")
      .eq("company_id", companyId)
      .eq("year", year)
      .eq("month", month)
    ...
    if (existingStats) {
      const tokensByModel =
        (existingStats.tokens_by_model as Record<string, number>) || {};
      const tokensByUsage =
        (existingStats.tokens_by_usage as Record<string, number>) || {};

      const modelKey = calculation.modelTier;
      tokensByModel[modelKey] =
        (tokensByModel[modelKey] || 0) + calculation.chargedTokens;
      tokensByUsage[usageType] =
        (tokensByUsage[usageType] || 0) + calculation.chargedTokens;

      await this.supabase
        .from("monthly_token_usage_stats")
        .update({
          total_articles_generated:
            (existingStats.total_articles_generated || 0) + 1,
          total_tokens_used:
            (existingStats.total_tokens_used || 0) + calculation.chargedTokens,
          total_cost_usd:
            Number(existingStats.total_cost_usd || 0) +
            calculation.chargedCostUsd,
  ...
  async checkAndResetMonthlyQuota(companyId: string): Promise<void> {
    const { data: subscription } = await this.supabase
      .from("company_subscriptions")
      .select(
        "id, current_period_end, monthly_token_quota, monthly_quota_balance",
      )
      .eq("company_id", companyId)
      .eq("status", "active")
      .single();
    ...
    // 免費方案（月配額為 0）不需要重置
    if (
      subscription.monthly_token_quota === 0 ||
    ...
    if (now > periodEnd) {
      ...
      await this.supabase.from("token_balance_changes").insert({
        ...
        amount:
          subscription.monthly_token_quota - subscription.monthly_quota_balance,
  ...
  async getCurrentBalance(companyId: string): Promise<{
    total: number;
    monthly: number;
    purchased: number;
  }> {
    ...
    if (error || !data) {
      ...
      if (errorMessage.includes("Insufficient balance")) {
        throw new InsufficientBalanceError(
          `餘額不足：需要 ${amount} tokens，請升級方案或購買 Credit`,
        );
      }
  ...
  private async logTokenUsage(params: {
    ...
    await this.supabase.from("token_usage_logs").insert({
      ...
      metadata:
        params.metadata as unknown as Database["public"]["Tables"]["token_usage_logs"]["Insert"]["metadata"],
    });
  }
  ...
  async checkAndReserveTokens(
    ...
    const { data, error } = (await this.supabase.rpc(
      "check_and_reserve_tokens",
      {
        p_company_id: companyId,
        p_job_id: jobId,
        p_article_count: articleCount,
        p_estimated_per_article: ESTIMATED_TOKENS_PER_ARTICLE,
      } as never,
    )) as {
      data: Array<{
        success: boolean;
    ...
    if (error) {
      console.error("[TokenBillingService] 預扣檢查失敗:", error);
    ...
    if (!result) {
      return {
        ...
        availableBalance: 0,
        totalReserved: 0,
        requiredAmount: articleCount * ESTIMATED_TOKENS_PER_ARTICLE,
        reservationId: null,
      };
    }
  ...
  async getActiveReservationsTotal(companyId: string): Promise<number> {
    const { data, error } = (await this.supabase.rpc(
      "get_active_reservations_total",
      { p_company_id: companyId } as never,
    )) as { data: number | null; error: { message: string } | null };

    if (error) {
      console.error("[TokenBillingService] 獲取預扣總額失敗:", error);
      return 0;
    }
  ...
  async releaseReservation(jobId: string): Promise<boolean> {
    ...
    if (error) {
      console.error("[TokenBillingService] 釋放預扣失敗:", error);
      return false;
    }
  ...
  async consumeReservation(jobId: string): Promise<boolean> {
    ...
    if (error) {
      console.error("[TokenBillingService] 消耗預扣失敗:", error);
      return false;
    }
  ...
  async getAvailableBalanceWithReservations(companyId: string): Promise<{
    total: number;
    reserved: number;
    available: number;
  }> {
    const balance = await this.getCurrentBalance(companyId);
    const reserved = await this.getActiveReservationsTotal(companyId);
