...
export class SubscriptionStrategy extends BasePaymentStrategy {
  ...
  protected async validateSpecific(context: PaymentContext): Promise<boolean> {
    ...
    if (!paymentData.subscriptionPlan || !validPlans.includes(paymentData.subscriptionPlan)) {
   ...
   * 處理首次付費獎勵
   */
  private async processFirstPaymentRewards(context: PaymentContext): Promise<void> {
    ...
    try {
      ...
      if (paymentData.referralCode) {
        await supabase.rpc('add_first_payment_bonus', {
          p_user_id: userId,
   ...
   * 發送歡迎郵件
   */
  private async sendWelcomeEmail(context: PaymentContext, subscription: any): Promise<void> {
    try {
   ...
   * 追蹤訂閱分析
   */
  private async trackSubscription(context: PaymentContext, subscription: any): Promise<void> {
    try {
