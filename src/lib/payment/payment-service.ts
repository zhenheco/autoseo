    ...
    try {
      ...
      if (status === "SUCCESS") {
        ...
        if (
          ...
          (orderData.payment_type as string) === "social_credit_package" &&
          orderData.related_id
        ) {
          // 處理社群發文點數套餐購買
          // 類型定義（social_credit_packages 表尚未在 database.types.ts 中）
          interface SocialCreditPackageRow {
            id: string;
            name: string;
            slug: string;
            credits: number;
          ...
          const { data: rawPackageData, error: packageError } =
            await this.supabase
              .from("social_credit_packages" as "token_packages") // 暫時類型斷言
              .select("*")
              .eq("id", orderData.related_id)
              .single();

          const packageData =
            rawPackageData as unknown as SocialCreditPackageRow | null;

          if (packageError || !packageData) {
            console.error("[PaymentService] 社群點數套餐不存在:", packageError);
            return { success: false, error: "社群點數套餐不存在" };
          }

          console.log("[PaymentService] 社群點數套餐資料:", {
            packageId: packageData.id,
            packageSlug: packageData.slug,
          ...
          // 查詢或建立公司社群點數餘額
          interface CompanySocialCreditsRow {
            id: string;
            company_id: string;
          ...
          const { data: rawExistingCredits, error: creditsError } =
            await this.supabase
              .from("company_social_credits" as "companies") // 暫時類型斷言
              .select("*")
              .eq("company_id", orderData.company_id)
              .maybeSingle();

          const existingCredits =
            rawExistingCredits as unknown as CompanySocialCreditsRow | null;

          if (creditsError) {
            console.error(
              "[PaymentService] 查詢社群點數餘額失敗:",
              creditsError,
            );
            return { success: false, error: "查詢社群點數餘額失敗" };
          }

          if (existingCredits) {
            // 更新現有餘額
            const newCreditsRemaining =
              existingCredits.credits_remaining + packageData.credits;

            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const { error: updateCreditsError } = await (this.supabase as any)
            ...
            if (updateCreditsError) {
              console.error(
                "[PaymentService] 更新社群點數餘額失敗:",
                updateCreditsError,
              );
              return { success: false, error: "更新社群點數餘額失敗" };
            }

            console.log("[PaymentService] 社群點數餘額已更新:", {
              creditsAdded: packageData.credits,
              previousBalance: existingCredits.credits_remaining,
              newBalance: newCreditsRemaining,
            });
          } else {
            // 建立新的點數餘額記錄
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const { error: insertCreditsError } = await (this.supabase as any)
              .from("company_social_credits")
              .insert({
                company_id: orderData.company_id,
            ...
            if (insertCreditsError) {
              console.error(
                "[PaymentService] 建立社群點數餘額失敗:",
                insertCreditsError,
              );
              return { success: false, error: "建立社群點數餘額失敗" };
            }

            console.log("[PaymentService] 社群點數餘額已建立:", {
              credits: packageData.credits,
            });
          }

          // 記錄購買到 purchased_social_credits（FIFO 追蹤）
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const { error: purchaseRecordError } = await (this.supabase as any)
            ...
            .insert({
              ...
              original_credits: packageData.credits,
              remaining_credits: packageData.credits,
              expires_at: null, // 永久有效
            });

          if (purchaseRecordError) {
            console.error(
              "[PaymentService] 記錄社群點數購買失敗:",
              purchaseRecordError,
            );
            // 不返回錯誤，因為主要邏輯已完成
          }

          console.log("[PaymentService] 社群點數套餐處理成功:", {
            packageSlug: packageData.slug,
            packageName: packageData.name,
            credits: packageData.credits,
          });
